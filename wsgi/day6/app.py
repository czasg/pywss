...
def userList(ctx: Ctx):
    size = int(ctx.queryParams().get("size", 1))
    ctx.write(list(DB.users.items())[:size])


def userRegister(ctx: Ctx):
    req = ctx.json()
    name = req.get("name")
    age = req.get("age")
    if not all((name, age)):
        ctx.write({"status": "error", "msg": "name/age不能为空"})
        return
    DB.add(name, age)
    ctx.write({"status": "success", "msg": "注册成功"})


def userLogin(ctx: Ctx):
    req = ctx.json()
    name = req.get("name")
    age = DB.get(name)
    if age is not None:
        ctx.setCookie("username", name)
        ctx.setCookie("age", age)
        ctx.write({"status": "success", "msg": "登录成功，请查看cookies"})
    else:
        ctx.write({"status": "error", "msg": f"登录失败，不存在该用户[{name}]"})
