...
- **快速**：纯手撸 socket，拒绝中间商赚取性能差价。（小白也能懂的源码）(有性能追求的同学，可以学学**Go**)
...
- **接口测试**：提供了 Http API 测试模块，不启动服务也能测试接口功能辣！
...
```python title="基于 next 实现 log 中间件" {6}
import pywss
import time
...
```python title="基于 next 实现 cors 中间件" {11}
import pywss
import time
