...
```
  ...
  <TabItem value="user.py" label="/api/v1/user.py（类视图）">
...
```
  ...
  <TabItem value="user_role.py" label="/api/v1/user_role.py（类app视图）">
...
```
  ...
  <TabItem value="permission.py" label="/api/v2/permission.py（函数视图）">
...
为了避免不同路由模块之间的冲突，`view_modules` 仅且支持指定当前调用模块下的子模块。

`view_modules` 会遍历指定**路由模块**，并读取子模块下的两个特定属性 `__view__` 和 `__route__`，然后通过这两个属性来加载视图模块。

同时，为了更加灵活的实现视图机制，`view_modules` 支持三种机制：

#### 类视图
类视图就是前章节中普通的视图机制。

#### 类app视图
在某些场景下，可能需要使用到 App 执行初始化。

`view_modules` 加入了类初始化函数的判断逻辑，即当类中指定 `def __init__(self, app: pywss.App)` 初始化函数时，
会将 app 作为参数传递执行初始化。

#### 函数视图
对于习惯函数式编程的同学来说，可能会更喜欢此种方式。

`view_modules` 同样会判断视图是否属于函数，如果是函数，则会创建 `party` 并将 App 作为参数传递给视图函数。
