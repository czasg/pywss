...
|**`ctx.file()`**|`func`|解析表单请求，并以**`Dict[str, pywss.File]`**形式返回|
|**`ctx.stream()`**|`func`|针对某些大文件场景，支持流式操作|
...
```python title="全局注册" showLineNumbers {11}
import pywss
import time
...
```python title="局部注册" showLineNumbers {12}
import pywss
import time
