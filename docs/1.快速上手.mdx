...
在 **`main.py`** 中我们可以看到，首先是在第7行初始化了一个`app`，并在第10行执行`app.run`启动 web 应用。

这两行代码，囊括了 app 从 **初始化 -> 启动** 的整个周期。

其次是**注册路由模块**，也就是第8、9行的注册路由方法，其中：
...
|**`ctx.json()`**|`func`|**解析请求**，并以 **JSON** 形式返回|
|**`ctx.form()`**|`func`|**解析请求**，并以 **FORM** 形式返回|
|**`ctx.file()`**|`func`|**解析请求**，并以 **`Dict[str, pywss.File]`** 形式返回|
...
具体实现流程此处不展开，感兴趣的同学可以阅读[源码](https://github.com/czasg/pywss)。
